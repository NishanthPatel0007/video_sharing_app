  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (!rateLimitResult.allowed) {
        throw new WorkerError('Rate limit exceeded', 429, {
...
async function checkRateLimit(request, clientIP, env) {
  const isUpload = request.method === 'PUT' || request.method === 'POST';
  const key = `${clientIP}:${isUpload ? 'upload' : 'download'}`;
  const limit = isUpload ? config.rateLimits.upload : config.rateLimits.download;
...
// Handle HEAD Request with Range Support
...
// Handle GET Request with Range Support and Streaming
...
// Handle PUT Request with Chunked Upload Support
...
async function handleSingleFileUpload(key, request, contentType, env) {
  ...
  await env.MY_BUCKET.put(key, data, {
    httpMetadata: {
      contentType,
      cacheControl: `public, max-age=${config.cacheDuration}`,
    },
    customMetadata: {
...
async function handleChunkedUpload(key, request, chunkInfo, env) {
  ...
  return jsonResponse({
    ...
    message: parseInt(partNumber) === parseInt(totalParts) - 1 
      ? 'All chunks uploaded and ready to combine'
...
async function cleanupChunks(key, uploadId, totalParts, env) {
  ...
  for (let i = 0; i < parts; i++) {
    try {
      await env.MY_BUCKET.delete(`${chunkFolder}/part${i}`);
    } catch (e) {
      console.error(`Failed to cleanup chunk ${i}:`, e);
    }
  }
}
